...
def run_symbol_analysis(board_name: str):
    ...
    try:
        ...
        ], cwd="ci", capture_output=True, text=True)
...
def load_and_summarize_results(board_name: str):
    ...
    try:
        ...
        for i, symbol in enumerate(summary['largest_symbols'][:5], 1):
            ...
            print(f"   {i}. {symbol['size']:6d} bytes - {name[:80]}{'...' if len(name) > 80 else ''}")
...
def main():
    ...
    if not boards_to_analyze:
        ...
        print("   Please compile some examples first using: uv run ci/ci-compile.py <board> --examples Blink")
        sys.exit(1)
    
    print(f"📋 Found {len(boards_to_analyze)} boards with build info: {', '.join(boards_to_analyze)}")
    ...
    print(f"📁 Detailed JSON files saved in .build/ directory")
