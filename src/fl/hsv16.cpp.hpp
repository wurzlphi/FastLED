...
static HSV16 RGBtoHSV16(const CRGB &rgb) {
    ...
    if (mx > 0) {
        ...
        if (delta == mx) {
            ...
            s = (fl::u16)(((uint32_t)delta * 65535 + (mx >> 1)) / mx);
        }
    }
...
static CRGB HSV16toRGB(const HSV16& hsv) {
    ...
    uint8_t R = map16_to_8(fl::u16(r1 + m));
    uint8_t G = map16_to_8(fl::u16(g1 + m));
    uint8_t B = map16_to_8(fl::u16(b1 + m));
