...
WaveSimulation2D::WaveSimulation2D(fl::u32 W, fl::u32 H, SuperSample factor,
                                   float speed, float dampening) {
    init(W, H, factor, speed, dampening);
}

void WaveSimulation2D::init(fl::u32 width, fl::u32 height, SuperSample factor,
                            float speed, int dampening) {
    mOuterWidth = width;
    mOuterHeight = height;
    mMultiplier = static_cast<fl::u32>(factor);
    mSim.reset(); // clear out memory first.
    fl::u32 w = width * mMultiplier;
    fl::u32 h = height * mMultiplier;
...
float WaveSimulation2D::getf(size_t x, size_t y) const {
    if (!has(x, y))
        return 0.0f;
    float sum = 0.0f;
    for (fl::u32 j = 0; j < mMultiplier; ++j) {
        for (fl::u32 i = 0; i < mMultiplier; ++i) {
...
fl::i16 WaveSimulation2D::geti16(size_t x, size_t y) const {
    if (!has(x, y))
        return 0;    
    fl::i32 sum = 0;
    fl::u8 mult = MAX(1, mMultiplier);
    for (fl::u32 j = 0; j < mult; ++j) {
        for (fl::u32 i = 0; i < mult; ++i) {
            fl::u32 xx = x * mult + i;
            fl::u32 yy = y * mult + j;
            fl::i32 pt = mSim->geti16(xx, yy);
            // fl::i32 ch_pt = mChangeGrid[(yy * mMultiplier) + xx];
            fl::i32 ch_pt = mChangeGrid(xx, yy);
    ...
    fl::i16 out = static_cast<fl::i16>(sum / (mult * mult));
    return out;
}

fl::i16 WaveSimulation2D::geti16Previous(size_t x, size_t y) const {
    if (!has(x, y))
        return 0;
    fl::i32 sum = 0;
    fl::u8 mult = MAX(1, mMultiplier);
...
bool WaveSimulation2D::geti16All(size_t x, size_t y, fl::i16 *curr,
                                 fl::i16 *prev, fl::i16 *diff) const {
    if (!has(x, y))
        return false;
    *curr = geti16(x, y);
...
fl::i8 WaveSimulation2D::geti8(size_t x, size_t y) const {
    return static_cast<fl::i8>(geti16(x, y) >> 8);
}

fl::u8 WaveSimulation2D::getu8(size_t x, size_t y) const {
    fl::i16 value = geti16(x, y);
    ...
    return static_cast<fl::u8>(((static_cast<fl::u16>(value) + 32768)) >> 8);
}
...
void WaveSimulation2D::seti16(size_t x, size_t y, fl::i16 v16) {
    if (!has(x, y))
        return;
    ...
    for (size_t j = 0; j < mult; ++j) {
        for (size_t i = 0; i < mult; ++i) {
            ...
            size_t yy = y * mult + j;
            if (mSim->has(xx, yy)) {
                fl::i16 &pt = mChangeGrid.at(xx, yy);
                if (pt == 0) {
...
void WaveSimulation2D::setf(size_t x, size_t y, float value) {
    ...
    fl::i16 v16 = wave_detail::float_to_fixed(value);
    seti16(x, y, v16);
}

void WaveSimulation2D::update() {
    const vec2<fl::i16> min_max = mChangeGrid.minMax();
    const bool has_updates = min_max != vec2<fl::i16>(0, 0);
    for (fl::u8 i = 0; i < mExtraFrames + 1; ++i) {
        if (has_updates) {
            // apply them
            const fl::u32 w = mChangeGrid.width();
            const fl::u32 h = mChangeGrid.height();
            for (fl::u32 x = 0; x < w; ++x) {
                for (fl::u32 y = 0; y < h; ++y) {
                    fl::i16 v16 = mChangeGrid(x, y);
                    if (v16 != 0) {
                        mSim->seti16(x, y, v16);
                    }
                }
...
fl::u32 WaveSimulation2D::getWidth() const { return mOuterWidth; }
fl::u32 WaveSimulation2D::getHeight() const { return mOuterHeight; }

void WaveSimulation2D::setExtraFrames(fl::u8 extra) { mExtraFrames = extra; }

WaveSimulation1D::WaveSimulation1D(fl::u32 length, SuperSample factor,
                                   float speed, int dampening) {
    init(length, factor, speed, dampening);
}

void WaveSimulation1D::init(fl::u32 length, SuperSample factor, float speed,
                            int dampening) {
    mOuterLength = length;
    mMultiplier = static_cast<fl::u32>(factor);
    mSim.reset(); // clear out memory first.
    ...
    // proportion to the supersampling factor.
    mExtraFrames = static_cast<fl::u8>(factor) - 1;
}
...
void WaveSimulation1D::setExtraFrames(fl::u8 extra) { mExtraFrames = extra; }
...
fl::i16 WaveSimulation1D::geti16(size_t x) const {
    if (!has(x))
        return 0;
    fl::u8 mult = MAX(1, mMultiplier);
    fl::i32 sum = 0;
    ...
    return static_cast<fl::i16>(sum / mult);
}

fl::i16 WaveSimulation1D::geti16Previous(size_t x) const {
    if (!has(x))
        return 0;
    fl::u8 mult = MAX(1, mMultiplier);
    fl::i32 sum = 0;
...
bool WaveSimulation1D::geti16All(size_t x, fl::i16 *curr, fl::i16 *prev,
                                 fl::i16 *diff) const {
    if (!has(x))
        return false;
    *curr = geti16(x);
...
fl::i8 WaveSimulation1D::geti8(size_t x) const {
    return static_cast<fl::i8>(geti16(x) >> 8);
}
...
//         }
//     }
//     return static_cast<fl::u8>(((static_cast<fl::u16>(value) + 32768)) >>
//     8);
// }

fl::u8 WaveSimulation1D::getu8(size_t x) const {
    fl::i16 value = geti16(x);
    if (mSim->getHalfDuplex()) {
        fl::u16 v2 = static_cast<fl::u16>(value);
        switch (mU8Mode) {
        case WAVE_U8_MODE_LINEAR:
...
void WaveSimulation1D::update() {
    mSim->update();
    for (fl::u8 i = 0; i < mExtraFrames; ++i) {
        mSim->update();
    }
}

fl::u32 WaveSimulation1D::getLength() const { return mOuterLength; }
