...
// fl::set_inlined<T, N, Allocator> - Inlined set implementation
// Stores the first N elements inline, then falls back to heap allocation
template <typename T, fl::size N, typename Allocator = fl::allocator<T>>
class set_inlined {
private:
    // Determine the appropriate allocator based on the input allocator
    template <typename A>
    struct allocator_rebind {
        using type = A;
    };
    
    // Special case: if using allocator_slab, rebind to allocator_inlined_slab_psram
    template <typename U, fl::size SLAB_SIZE>
    struct allocator_rebind<fl::allocator_slab<U, SLAB_SIZE>> {
        using type = fl::allocator_inlined_slab_psram<T, N, SLAB_SIZE>;
    };
    
    using InlinedAllocator = typename allocator_rebind<Allocator>::type;
    using VectorType = fl::HeapVector<T, InlinedAllocator>;
...
public:
    ...
    set_inlined(const set_inlined& other) : m_data(other.m_data) {}
    
    set_inlined(set_inlined&& other) : m_data(fl::move(other.m_data)) {}
    
    set_inlined& operator=(const set_inlined& other) {
        if (this != &other) {
            m_data = other.m_data;
        }
        return *this;
    ...
    set_inlined& operator=(set_inlined&& other) {
        if (this != &other) {
            m_data = fl::move(other.m_data);
        }
        return *this;
    }
    
    // Initializer list constructor
    set_inlined(fl::initializer_list<T> init) {
    ...
    size_type inlined_capacity() const { return N; }
    ...
    fl::pair<iterator, bool> insert(const T& value) {
        auto it = find(value);
        if (it != end()) {
            return fl::make_pair(it, false); // Already exists
        }
        
        m_data.push_back(value);
        return fl::make_pair(m_data.end() - 1, true);
    }
    
    fl::pair<iterator, bool> insert(T&& value) {
        auto it = find(value);
        if (it != end()) {
            return fl::make_pair(it, false); // Already exists
        }
        
        m_data.push_back(fl::move(value));
        return fl::make_pair(m_data.end() - 1, true);
    }
    ...
    fl::pair<iterator, bool> emplace(Args&&... args) {
        // Create a temporary to check if it already exists
        T temp_value(fl::forward<Args>(args)...);
        auto it = find(temp_value);
        if (it != end()) {
            return fl::make_pair(it, false); // Already exists
        }
        
        m_data.push_back(fl::move(temp_value));
        return fl::make_pair(m_data.end() - 1, true);
    }
