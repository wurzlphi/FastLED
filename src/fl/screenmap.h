...
class ScreenMap {
  public:
    ...
    ScreenMap(fl::u32 length, float mDiameter = -1.0f);

    ScreenMap(const vec2f *lut, fl::u32 length, float diameter = -1.0);
    ...
    const vec2f &operator[](fl::u32 x) const;
    ...
    vec2f mapToIndex(fl::u32 x) const;
