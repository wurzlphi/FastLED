...
class XYRasterU8Sparse {
  public:
    ...
    XYRasterU8Sparse(int width, int height) {
        setBounds(rect<fl::i16>(0, 0, width, height));
    }
    ...
    void rasterize(const vec2<fl::i16> &pt, fl::u8 value) {
    ...
    void setBounds(const rect<fl::i16> &bounds) {
        mAbsoluteBounds = bounds;
        mAbsoluteBoundsSet = true;
    }

    using iterator = fl::HashMap<vec2<fl::i16>, fl::u8>::iterator;
    using const_iterator = fl::HashMap<vec2<fl::i16>, fl::u8>::const_iterator;
    ...
    void rasterize_internal(const Tile2x2_u8 &tile,
                            const rect<fl::i16> *optional_bounds = nullptr);
    ...
    Pair<bool, fl::u8> at(u16 x, u16 y) const {
        const fl::u8 *val = mSparseGrid.find_value(vec2<fl::i16>(x, y));
        if (val != nullptr) {
            return {true, *val};
        }
        return {false, 0};
    ...
    rect<fl::i16> bounds_pixels() const {
        int min_x = 0;
        ...
        return rect<fl::i16>(min_x, min_y, max_x + 1, max_y + 1);
    }
    ...
    void write(const vec2<fl::i16> &pt, fl::u8 value) {
  ...
  private:
    ...
    HashMap<vec2<fl::i16>, fl::u8 *, FastHashKey, EqualToKey, kMaxCacheSize>
        mCache;
    fl::rect<fl::i16> mAbsoluteBounds;
...
class XYRasterSparse_CRGB {
  public:
    ...
    void rasterize(const vec2<fl::i16> &pt, const CRGB &color) {
        write(pt, color);
    }
    ...
    using iterator = fl::HashMap<vec2<fl::i16>, CRGB>::iterator;
    using const_iterator = fl::HashMap<vec2<fl::i16>, CRGB>::const_iterator;
    ...
    Pair<bool, CRGB> at(u16 x, u16 y) const {
        const CRGB *val = mSparseGrid.find_value(vec2<fl::i16>(x, y));
        if (val != nullptr) {
            return {true, *val};
        }
    ...
    void write(const vec2<fl::i16> &pt, const CRGB &color) {
  ...
  private:
    ...
    HashMapLarge mSparseGrid;
    // Small cache for the last N writes to help performance.
    HashMap<vec2<fl::i16>, CRGB *, FastHashKey, EqualToKey, kMaxCacheSize>
        mCache;
    fl::rect<fl::i16> mAbsoluteBounds;
    bool mAbsoluteBoundsSet = false;
};
