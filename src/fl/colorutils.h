...
void fadeLightBy(CRGB *leds, u16 num_leds, uint8_t fadeBy);

/// @copydoc fadeLightBy()
void fade_video(CRGB *leds, u16 num_leds, uint8_t fadeBy);
...
void nscale8_video(CRGB *leds, u16 num_leds, uint8_t scale);
...
/// @param num_leds the number of LEDs to fade
/// @param fadeBy how much to fade each LED
void fadeToBlackBy(CRGB *leds, u16 num_leds, uint8_t fadeBy);

/// @copydoc fadeToBlackBy()
void fade_raw(CRGB *leds, u16 num_leds, uint8_t fadeBy);
...
/// @param num_leds the number of LEDs to scale
/// @param scale how much to scale each LED
void nscale8(CRGB *leds, u16 num_leds, uint8_t scale);
...
void fadeUsingColor(CRGB *leds, u16 numLeds, const CRGB &colormask);
...
CRGB *blend(const CRGB *src1, const CRGB *src2, CRGB *dest, u16 count,
            fract8 amountOfsrc2);

/// @copydoc blend(const CRGB*, const CRGB*, CRGB*, u16, fract8)
/// @param directionCode the direction to travel around the color wheel
CHSV *blend(const CHSV *src1, const CHSV *src2, CHSV *dest, u16 count,
            fract8 amountOfsrc2,
            TGradientDirectionCode directionCode = SHORTEST_HUES);
...
/// @param amountOfOverlay the fraction of overlay to blend into existing
void nblend(CRGB *existing, const CRGB *overlay, u16 count,
            fract8 amountOfOverlay);

/// @copydoc nblend(CRGB*, const CRGB*, u16, fract8)
/// @param directionCode the direction to travel around the color wheel
void nblend(CHSV *existing, const CHSV *overlay, u16 count,
            fract8 amountOfOverlay,
            TGradientDirectionCode directionCode = SHORTEST_HUES);
...
class CHSVPalette256 {
  public:
    ...
    bool operator==(const CHSVPalette256 &rhs) const {
        ...
        if (p == q)
            return true;
        for (u16 i = 0; i < (sizeof(entries)); ++i) {
            if (*p != *q)
                return false;
            ++p;
            ++q;
...
/// @brief Same as ColorFromPalette, but with u16 `index` to give greater
/// precision.
...
// @author https://github.com/generalelectrix
CRGB ColorFromPaletteExtended(const CRGBPalette256 &pal, u16 index,
                              uint8_t brightness = 255,
                              TBlendType blendType = LINEARBLEND);
...
void fill_palette(CRGB *L, u16 N, uint8_t startIndex, uint8_t incIndex,
...
template <typename PALETTE>
void fill_palette_circular(CRGB *L, u16 N, uint8_t startIndex,
                           const PALETTE &pal, uint8_t brightness = 255,
                           TBlendType blendType = LINEARBLEND,
                           bool reversed = false) {
    if (N == 0)
...
void map_data_into_colors_through_palette(
    uint8_t *dataArray, u16 dataCount, CRGB *targetColorArray,
...
/// @param gamma the gamma value to apply
void napplyGamma_video(CRGB *rgbarray, u16 count, float gamma);
...
/// @param gammaG the gamma value to apply to the CRGB::green channel
/// @param gammaB the gamma value to apply to the CRGB::blue channel
void napplyGamma_video(CRGB *rgbarray, u16 count, float gammaR,
