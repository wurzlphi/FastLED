...
template <fl::u8 DATA_PIN, fl::u8 CLOCK_PIN, fl::u32 SPI_SPEED>
...
public:
	...
	static void writeByteNoWait(fl::u8 b) __attribute__((always_inline)) { writeByte(b); }
	/// Write a single byte over SPI and wait afterwards. 
	static void writeBytePostWait(fl::u8 b) __attribute__((always_inline)) { writeByte(b); wait(); }
...
private:
	...
	static void writeByte(fl::u8 b, clock_ptr_t clockpin, data_ptr_t datapin)  {
	...
	static void writeByte(fl::u8 b, data_ptr_t datapin,
...
public:
	...
	template <fl::u8 BIT> __attribute__((always_inline, hot)) inline static void writeBit(fl::u8 b) {
		//cli();
		if(b & (1 << BIT)) {
			FastPin<DATA_PIN>::hi();
#ifdef ESP32
...
private:
	...
	template <fl::u8 BIT> FASTLED_FORCE_INLINE static void writeBit(fl::u8 b, clock_ptr_t clockpin, data_ptr_t datapin) {
		if(b & (1 << BIT)) {
	...
	/// combinations
	template <fl::u8 BIT> FASTLED_FORCE_INLINE static void writeBit(fl::u8 b, data_ptr_t clockdatapin,
...
public:
	...
	/// @copydetails AVRSoftwareSPIOutput::writeBytesValue(fl::u8, int)
	...
	template <fl::u8 FLAGS, class D, EOrder RGB_ORDER>  __attribute__((noinline)) void writePixels(PixelController<RGB_ORDER> pixels, void* context = NULL) {
