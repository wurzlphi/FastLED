...
void set_max_power_in_volts_and_milliamps( fl::u8 volts, uint32_t milliamps);
...
fl::u8 calculate_max_brightness_for_power_mW(const CRGB* ledbuffer, uint16_t numLeds, fl::u8 target_brightness, uint32_t max_power_mW);
...
/// @returns a limited brightness value. No higher than the target brightness,
/// but may be lower depending on the power limit.
fl::u8 calculate_max_brightness_for_power_vmA(const CRGB* ledbuffer, uint16_t numLeds, fl::u8 target_brightness, uint32_t max_power_V, uint32_t max_power_mA);
