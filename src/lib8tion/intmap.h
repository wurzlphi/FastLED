...
/// `102 / 255`. Using `map8_to_16(fl::u8)` to convert that to a 16-bit
/// unsigned integer would give you `26,214 / 65,535`, exactly 40% through the
/// larger range.
///
/// @{

LIB8STATIC_ALWAYS_INLINE uint16_t map8_to_16(fl::u8 x) {
...
LIB8STATIC_ALWAYS_INLINE fl::u8 map16_to_8(uint16_t x) {
...
LIB8STATIC_ALWAYS_INLINE uint32_t map8_to_32(fl::u8 x) {
